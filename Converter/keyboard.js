...
function replaceTheLastCharWithClickedKey(char){
    // console.log("KEY is CLICKED....YAHOOOOO AGAIN")
    console.log(benData);
    let lastWordInArray = benData.slice(-1);
    console.log(lastWordInArray[0]);
    let lastWordWithoutFinalCharacter=lastWordInArray[0].slice(0,-1);
    console.log("last Word without final character",lastWordWithoutFinalCharacter);
    benData.pop();
    console.log(benData);
    benData.push(lastWordWithoutFinalCharacter+char);
...
btnSubmit.addEventListener("click",async ()=>{
    ...
    let benDataText=benText.innerHTML;
    ...
    benDataArray = await bengTextToArray(benDataText)
    engDataArray = await bengTextToArray(engDataText)
...
async function bengTextToArray(benDataText){
